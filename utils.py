...
# 属于同一类的节点添加邻接关系
def get_cluster_adjacency_matrix(A, cluster_res_filename, num_of_vertices, num_of_clusters):
    df = pd.read_csv(cluster_res_filename, sep='\t', skiprows=1, names=['node', 'kshape'])
    print(df.columns)

    # 遍历数据框，根据聚类信息将属于同一类的节点之间设置为相邻
    for i in range(num_of_vertices):
        for j in range(i + 1, num_of_vertices):
            if df.loc[i, 'kshape'] == df.loc[j, 'kshape']:
                A[i, j] = 1
                A[j, i] = 1  # 因为是无向图，所以需要对称设置
    return A

# 构造语义邻接矩阵，基于聚类结果
def get_semantic_adjacency_matrix(cluster_res_filename, num_of_vertices, num_of_clusters):
    df = pd.read_csv(cluster_res_filename, sep='\t', skiprows=1, names=['node', 'kshape'])
    print(df.columns)
...
def norm_Adj(W):
    ...
    N = W.shape[0]
    W = W + np.identity(N)  # 为邻接矩阵加上自连接：np.identity返回的是N × N的主对角线为1，其余地方为0的数组
    # R = np.sum(W, axis=1)
    # D = np.diag(1.0/np.sqrt(R))
    # norm_Adj_matrix = np.dot(np.dot(D, W), D)
    D = np.diag(1.0 / np.sum(W, axis=1))  # diag()函数是以一维数组的形式返回方阵的对角线元素
    norm_Adj_matrix = np.dot(D, W)

    return norm_Adj_matrix

# make_model：构造对称归一化的拉普拉斯矩阵
def sym_norm_Adj(W):
    '''
...
def compute_val_loss(net, val_loader, criterion, epoch, process = 'first stage'):
    '''
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(val_loader):
            ...
            # predict_output = net(encoder_inputs, decoder_inputs, process)
...
def predict_and_save_results(net, data_loader, data_target_tensor, epoch, _max, _min, params_path, type, process = 'second stage'):
    '''
    for transformerGCN
    :param net: nn.Module
